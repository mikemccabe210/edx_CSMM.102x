...
#    assert len(args) == 5, 'Not enough args'
        ...
        lam = float(args[0])
        sigma2 = float(args[1])
        x_train = np.genfromtxt(args[2], delimiter = ',')
        y_train = np.genfromtxt(args[3], delimiter = ',')
        x_test = np.genfromtxt(args[4], delimiter = ',')

    cls = ridge_regression(lam, sigma2)
    cls.fit(x_train, y_train)
#    print(cls.weights)
    np.savetxt('wRR_%s.csv' % (lam), cls.weights, delimiter = ',')
#    print(cls.active_learn(x_test))
    np.savetxt('active_%s_%s.csv' % (lam, sigma2), cls.active_learn(x_test), delimiter = ',')
